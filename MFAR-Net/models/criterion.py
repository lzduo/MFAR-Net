...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses, iou_type, alpha=0.2, gamma=2.0, eos_coef=1e-4, num_classes=80):
        ...
        self.generate_iou_vfl = IouLoss("IoU").requires_grad_(False)
        self.alpha = alpha
        self.gamma = gamma
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        _, ious = self.generate_iou_vfl(box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes), True, False)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_iou = 1 - torch.diag(self.generate_iou_loss(box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes),
                                                         True, False)[-1])
...
def build_criterion(num_classes, matcher, weight_dict, iou_type, eos_coef, losses):
    return SetCriterion(matcher=matcher,
