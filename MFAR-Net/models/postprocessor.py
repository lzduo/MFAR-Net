...
class PostProcessor(nn.Module):
    ...
    def forward(self, outputs, orig_target_sizes):
        ...
        # print(bbox_pred, orig_target_sizes.repeat(1, 2).unsqueeze(1))
        ...
        if self.remap_mscoco_category:
            from ..datasets.coco_util import mscoco_label2category
...
def build_postprocessor(args):
    postprocessor = PostProcessor(num_classes=args.num_classes,
                                  use_focal_loss=args.use_focal_loss,
                                  num_top_queries=300,
                                  remap_mscoco_category=False
                                  )

    # return {'bbox': postprocessor}
