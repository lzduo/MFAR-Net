...
class GradCAM:
    def __init__(self, model, target_layers, reshape_transform=None, use_cuda=False):
        ...
        self.reshape_transform = reshape_transform
        self.cuda = use_cuda
        self.activations_and_grads = ActivationsAndGradients(target_layers, reshape_transform)
    ...
    def get_loss(output, target_category):
        ...
        for n in range(output.size()[1]):
            for i in range(len(target_category)):
                loss = loss + output[i, n, target_category[i]]
        return loss
    ...
    def clean_list(self,):
        self.activations_and_grads.clean_list()

    def __call__(self, input_tensor, output, target_category=None):
        ...
        if isinstance(target_category, int):
            target_category = [target_category] * input_tensor.size(0)   # <--------todo:changed:-1

        if target_category is None:
            target_category = np.argmax(output.cpu().data.numpy(), axis=-1)
            print(f"category id: {target_category}")
        else:
            assert (len(target_category) == input_tensor.size(0))    # <--------todo:changed:-1
